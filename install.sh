...
_load() {
	local _dir="/etc/v2ray/guyezi/v2ray/src/"
	. "${_dir}$@"
}
_sys_timezone() {
	IS_OPENVZ=
...
path_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认: [${cyan}guyezi$none]):")" path
		[[ -z $path ]] && path="guyezi"
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}guyezi.com$none)"): " sspass
		[ -z "$sspass" ] && sspass="guyezi.com"
		case $sspass in
		*[/$]*)
			echo
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		...
		mkdir -p /etc/v2ray/guyezi/v2ray
		cp -rf $(pwd)/* /etc/v2ray/guyezi/v2ray
	else
		pushd /tmp
		git clone https://github.com/guyezi/v2ray -b "$_gitbranch" /etc/v2ray/guyezi/v2ray --depth=1
		popd

	fi

	if [[ ! -d /etc/v2ray/guyezi/v2ray ]]; then
		echo
...
config() {
	cp -f /etc/v2ray/guyezi/v2ray/config/backup.conf $backup
...
backup_config() {
	...
	if [[ $shadowsocks ]]; then
		sed -i "42s/=/=true/; 45s/=6666/=$ssport/; 48s/=guyezi.com/=$sspass/; 51s/=chacha20-ietf/=$ssciphers/" $backup
	fi
	[[ $v2ray_transport == [45] ]] && sed -i "36s/=guyezi.com/=$domain/" $backup
	[[ $caddy ]] && sed -i "39s/=/=true/" $backup
	[[ $ban_ad ]] && sed -i "54s/=/=true/" $backup
	if [[ $is_path ]]; then
		sed -i "57s/=/=true/; 60s/=guyezi/=$path/" $backup
