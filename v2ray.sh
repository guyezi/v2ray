...
backup="/etc/v2ray/guyezi_v2ray_backup.conf"

if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/guyezi/v2ray ]]; then

	. $backup

elif [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f /etc/v2ray/guyezi_v2ray_backup.txt && -d /etc/v2ray/guyezi/v2ray ]]; then

	. /etc/v2ray/guyezi/v2ray/tools/v1xx_to_v3xx.sh
...
if [[ $mark != "v3" ]]; then
	. /etc/v2ray/guyezi/v2ray/tools/v3.sh
fi
...
v2ray_client_config="/etc/v2ray/guyezi_v2ray_config.json"
...
. /etc/v2ray/guyezi/v2ray/src/init.sh
systemd=true
# _test=true
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}guyezi.com$none)"): " new_sspass
		[ -z "$new_sspass" ] && new_sspass="guyezi.com"
		case $new_sspass in
		*[/$]*)
			echo
...
tls_config() {
	while :; do
		...
		read -p "(例如：guyezi.com): " new_domain
...
path_config() {
	echo
	while :; do
		echo -e "请输入想要 ${magenta}用来分流的路径$none , 例如 /guyezi , 那么只需要输入 guyezi 即可"
		read -p "$(echo -e "(默认: [${cyan}guyezi$none]):")" new_path
		[[ -z $new_path ]] && new_path="guyezi"
...
blocked_hosts() {
	...
	while :; do
		...
		echo
		echo "反馈问题或请求拦截更多域名: https://github.com/guyezi/v2ray/issues"
		echo
	...
	while :; do
		...
		if [[ -z $is_xshell ]]; then
			...
			if [[ $is_xshell == [yY] ]]; then
				...
				echo
				# sz /etc/v2ray/guyezi_v2ray.zip
				local tmpfile="/tmp/guyezi_v2ray_config_$RANDOM.json"
				...
				echo
				# echo -e "$yellow 解压密码 = ${cyan}guyezi.com$none"
				# echo
...
get_v2ray_config_info_link() {
	...
	echo
	create_v2ray_config_text >/tmp/guyezi_v2ray.txt
	local random=$(echo $RANDOM-$RANDOM-$RANDOM | base64 -w 0)
	local link=$(curl -s --upload-file /tmp/guyezi_v2ray.txt "https://transfer.sh/${random}_233v2_v2ray.txt")
	if [[ $link ]]; then
		echo
		echo "---------- V2Ray 配置信息链接-------------"
		echo
...
update_v2ray.sh() {
	if [[ $_test ]]; then
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/guyezi/v2ray/test/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	else
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/guyezi/v2ray/master/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	fi
	...
	if [[ $latest_version == $_version ]]; then
		...
		echo
		cd /etc/v2ray/guyezi/v2ray
		git pull
		cp -f /etc/v2ray/guyezi/v2ray/v2ray.sh $_v2ray_sh
		chmod +x $_v2ray_sh
		echo
...
menu() {
	...
	while :; do
		...
		echo
		echo "反馈问题: https://github.com/guyezi/v2ray/issues"
		echo
